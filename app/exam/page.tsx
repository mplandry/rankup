...
export default function ExamPage() {
  ...
  const sessionQsRef = useRef<SessionQuestion[]>([]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    let correct = 0;
    qs.forEach((sq, i) => { if (ans[i] === sq.correctLetter) correct++; });
    const pct = Math.round((correct / qs.length) * 100);
    setScore(pct);
    if (user) {
      await supabase.from("exam_sessions").insert({
        user_id: user.id, mode: "exam", score: pct,
        total_questions: qs.length, completed_at: new Date().toISOString(), time_taken: 0,
      });
    }
    setState("results");
  }, [user]);
      ...
      <div style={{ display: "flex" }}>
        ...
        <div style={{ marginLeft: "var(--sidebar-w)", flex: 1, padding: "36px 40px", maxWidth: 700 }}>
          ...
          <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, padding: "28px 32px", marginBottom: 24 }}>
            ...
            {[
              "90 questions selected randomly from the question bank",
              "90 minute time limit",
              "Score of 70% or higher is considered passing",
              `${questions.length} questions available in the pool`,
            ].map((rule, i) => (
      ...
      <div style={{ display: "flex" }}>
        ...
        <div style={{ marginLeft: "var(--sidebar-w)", flex: 1, padding: "36px 40px", maxWidth: 800 }}>
          ...
          <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, padding: "28px 32px" }}>
            ...
            {sq.choices.map(opt => {
              const sel = answers[idx] === opt.letter;
              return (
                <div key={opt.letter} onClick={() => handleAnswer(opt.letter)}
                  style={{ display: "flex", gap: 12, padding: "13px 16px", marginBottom: 10, borderRadius: 8, border: `1px solid ${sel ? "var(--red)" : "var(--border)"}`, background: sel ? "var(--red-light)" : "var(--white)", cursor: "pointer", transition: "all 0.1s" }}>
                  <span style={{ fontWeight: 700, color: sel ? "var(--red)" : "var(--text-muted)", width: 20, flexShrink: 0 }}>{opt.letter}</span>
      ...
      <div style={{ display: "flex" }}>
        ...
        <div style={{ marginLeft: "var(--sidebar-w)", flex: 1, padding: "36px 40px", maxWidth: 700 }}>
          ...
          <div style={{ background: "var(--white)", border: "1px solid var(--border)", borderRadius: 12, padding: "40px", textAlign: "center", marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: score >= 70 ? "var(--green)" : "var(--red)", marginBottom: 8 }}>{score >= 70 ? "✓ Passing Score" : "✗ Below Passing"}</div>
            <div style={{ fontSize: 14, color: "var(--text-muted)" }}>Passing threshold: 70%</div>
          </div>
          <div style={{ display: "flex", gap: 12 }}>
            <button onClick={() => { setState("configure"); setSessionQs([]); setAnswers({}); }}
              style={{ padding: "12px 28px", background: "var(--red)", color: "#fff", border: "none", borderRadius: 8, fontSize: 14, fontWeight: 700, cursor: "pointer" }}>Try Again</button>
