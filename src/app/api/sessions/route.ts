...
export async function POST(request: Request) {
  ...
  const { mode, filters }: { mode: 'study' | 'exam'; filters?: StudyFilters } = body
  ...
  let query = supabase
    .from('questions')
    .select('id, question_text, answer_a, answer_b, answer_c, answer_d, correct_answer, explanation, book_title, edition, chapter, topic, page_start, page_end, difficulty, study_eligible, exam_eligible, source_id, is_active, created_by, created_at, updated_at')
    .eq('is_active', true)
  ...
  const questionCount = mode === 'exam' ? EXAM_QUESTION_COUNT : (filters?.question_count ?? 20)
  ...
  const { error: sqErr } = await supabase.from('exam_session_questions').insert(sessionQuestions)
  if (sqErr) {
    return NextResponse.json({ error: 'Failed to create session questions' }, { status: 500 })
  }
...
export async function PATCH(request: Request) {
  ...
  if (!session_question_id) {
    return NextResponse.json({ error: 'Missing session_question_id' }, { status: 400 })
  }
