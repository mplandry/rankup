    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
          <div>
            <h2 className="text-xl font-bold text-gray-900">{student.full_name || "Student Details"}</h2>
        ...
        <div className="p-6 space-y-6">
          {/* Basic Info */}
          <div>
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wide mb-3">Basic Information</h3>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="font-semibold">{student.exam_type === "captain" ? "Captain" : "Lieutenant"}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Last Active</div>
                <div className="font-semibold">{timeAgo(student.last_sign_in_at || null)}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Member Since</div>
                <div className="font-semibold">{new Date(student.created_at).toLocaleDateString()}</div>
              </div>
            </div>
          </div>
          ...
          {/* Performance Stats */}
          <div>
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wide mb-3">Performance</h3>
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-blue-50 rounded-lg p-4">
                <div className="text-xs text-blue-600 font-semibold">Total Sessions</div>
                <div className="text-2xl font-bold text-blue-900">{stats?.total_sessions || 0}</div>
              </div>
              <div className="bg-green-50 rounded-lg p-4">
                <div className="text-xs text-green-600 font-semibold">Avg Score</div>
                <div className="text-2xl font-bold text-green-900">{stats?.avg_score_percent || 0}%</div>
              </div>
              <div className="bg-purple-50 rounded-lg p-4">
                <div className="text-xs text-purple-600 font-semibold">Best Score</div>
                <div className="text-2xl font-bold text-purple-900">{stats?.best_score_percent || 0}%</div>
              </div>
            </div>
          </div>
          ...
          {stats && (
            <div>
              <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wide mb-3">Question History</h3>
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold text-green-600">{stats.total_correct}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-600">Incorrect Answers</span>
                  <span className="font-semibold text-red-600">{stats.total_questions - stats.total_correct}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-600">Last Session</span>
                  <span className="font-semibold">
                    {stats.last_session_at ? new Date(stats.last_session_at).toLocaleDateString() : "Never"}
                  </span>
                </div>
              </div>
            </div>
...
export default function StudentsTable({
  ...
  const filteredStudents = useMemo(() => {
    ...
    if (performanceFilter !== "all") {
      filtered = filtered.filter(s => {
        ...
        if (performanceFilter === "nosessions") return !s.user_stats_cache?.total_sessions;
        return true;
      });
    }
  ...
  async function handleEmailInactive() {
    ...
    try {
      const res = await fetch("/api/students/email-inactive", { method: "POST" });
    ...
    <>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {(searchQuery || examFilter !== "all" || activityFilter !== "all" || performanceFilter !== "all") && (
            <button
              onClick={() => {
                setSearchQuery("");
                setExamFilter("all");
