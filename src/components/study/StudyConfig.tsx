...
export default function StudyConfig({ books, chapters, topics }: Props) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-8 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <SelectField label="Book / Reference" value={book} onChange={setBook} placeholder="All books">
          {books.map((b) => <option key={b} value={b}>{b}</option>)}
        </SelectField>

        <SelectField label="Chapter" value={chapter} onChange={setChapter} placeholder="All chapters">
          {chapters.map((c) => <option key={c} value={c}>{c}</option>)}
        </SelectField>

        <SelectField label="Topic" value={topic} onChange={setTopic} placeholder="All topics">
          {topics.map((t) => t && <option key={t} value={t}>{t}</option>)}
        </SelectField>

        <SelectField label="Difficulty" value={difficulty} onChange={setDifficulty} placeholder="All difficulties">
