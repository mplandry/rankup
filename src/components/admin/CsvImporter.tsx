...
export default function CsvImporter() {
  ...
  const [importResult, setImportResult] = useState<ImportQualityResult | null>(
    null,
  );
  ...
  if (importResult) {
    const answerDist = importResult.answer_distribution || {
      A: 25,
      B: 25,
      C: 25,
      D: 25,
    };
    const { A, B, C, D } = answerDist;
    const isSkewed = Math.max(A, B, C, D) > 30 || Math.min(A, B, C, D) < 20;
    const skewedLetter = isSkewed
      ? (["A", "B", "C", "D"] as const).find(
          (l) => answerDist[l] === Math.max(A, B, C, D),
        )
      : null;
